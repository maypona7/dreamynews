  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {post.eventAt && (
          <span
            className="badge shrink-0 bg-brand-50 text-brand-700 ring-1 ring-brand-200/80"
            title="행사일"
            aria-label={`행사일 ${formatEventDate(post.eventAt)}`}
          >
            <span aria-hidden="true">📅</span>
            <time dateTime={new Date(post.eventAt).toISOString().slice(0, 10)}>
      ...
      <h3
        className={clsx(
          "text-lg font-semibold text-brand-950 line-clamp-2",
          isUnread && "text-brand-900",
        )}
      >
        {post.title}
      </h3>
      <p className="text-sm text-brand-700 line-clamp-3">{post.excerpt}</p>
